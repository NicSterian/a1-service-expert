...
export function QuoteEditor({ id }: { id: number }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        toast.error((err as Error).message ?? 'Failed to load quote draft');
      } finally {
        setLoading(false);
      }
    };
  ...
  const issue = async () => {
    ...
    if (!customer.name || customer.name.trim() === '') {
      toast.error('Customer name is required to issue quote');
      return;
    }
    try {
      await save();
      ...
      toast.error((err as Error).message ?? 'Failed to issue quote');
    }
  };
  ...
  const cancel = () => {
    navigate('/admin/financial?tab=quotes');
  };

  const deleteDraft = async () => {
    if (!doc) return;
    const ok = window.confirm('Delete this quote draft? This action cannot be undone.');
    if (!ok) return;
    try {
  ...
  return (
    <div className="rounded-3xl border border-slate-700 bg-slate-900 p-6">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-slate-400">Quote Draft - {doc.number}</h3>
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-slate-700 bg-slate-800 p-4 text-sm text-slate-200">
            <div className="grid grid-cols-1 gap-2">
              ...
              <input type="date" className="rounded border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-white" value={validUntil} onChange={(e) => setValidUntil(e.target.value)} />
              <label className="mt-3 text-xs text-slate-400">Notes</label>
              <textarea rows={4} className="rounded border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-white" value={notes} onChange={(e) => setNotes(e.target.value)} />
            </div>
          </div>
